...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $updatedate = "UPDATE user SET Name = '$name', Username = '$username', Password = '$password' WHERE ID = '$id'";
    $executeup = mysqli_query($conn, $updatedate);

    if($executeup){
        $sqlsearch = "SELECT * FROM user ORDER BY ID DESC LIMIT $limit";
        $resultsearch = mysqli_query($conn, $sqlsearch);
        echo '<p class="success" style="margin-bottom: -2%; margin-top: 2%;">Account Successfully Updated</p>';
