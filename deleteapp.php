...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $appid = validate($_POST['appid']);
    $email = validate($_POST['email']);

    $deleteapp ="DELETE FROM overall_appointments WHERE ID = '$appid'";
    $deleteappexe = mysqli_query($conn, $deleteapp);
    ...
    if($deleteappexe){
        ...
        try {
            ...
            $mail->isHTML(true);
            $mail->Subject = 'Dropped Appointment';
            $mail->Body    = '
            <html>
            <head>
              <style>
